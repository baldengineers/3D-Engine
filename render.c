        ...
        for ( w = xMin; w <= xMax; w++) //replace with yMinF, yMaxF when working
        {
            for (h = yMinF; h <= yMaxF; h++) //replace with xMin, xMax when working
            {
                ...
                if (h >= 0 && h < H && w >= 0 && w < W)
                {
                    ...
                    vector va = faceVertices[i][0];
                    vector vb = faceVertices[i][1];
                    vector vc = faceVertices[i][2];
                    vector p = {w, h, 0};
                    vector v0 = subtractVector(vb,va);
                    vector v1 = subtractVector(vc,va);
                    vector v2 = subtractVector(p,va);
                    ...
                    double l0 = (d11 * d20 - d01 * d21) / denom;
                    double l1 = (d00 * d21 - d01 * d20) / denom;
                    double l2 = 1.0 - l0 - l0;
                    
                    
                    
                    double pointDepth = 1 / ( (1/faceVertices[i][0].z) * l0 + (1/faceVertices[i][1].z) * l1 + (1/faceVertices[i][2].z) * l2 );

                    if (pointInTriangle(pixel, a, b, c))
                    {
                        if (pointDepth < depthBuffer[w][h] && va.z > 0 && pointDepth > 5) 
                        {
